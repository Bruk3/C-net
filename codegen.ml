 ...
 *******************************************************************************)
  ...
  *)
  ...
  let build_function_body (fdecl : sfunc) =
    ...
    (* scope : ((A.vdecl * L.llvalue) StringMap.t) list  *)
    let rec stmt builder scope = function
      | SExpr e -> ignore(expr builder e scope); builder
      | SReturn e -> ignore(match fdecl.styp with
          (* Special "return nothing" instr *)
            A.Void -> L.build_ret_void builder
          (* Build return statement *)
          | _ -> L.build_ret (expr builder e scope) builder ); builder

      | _ -> codegen_err "Unimplemented statement type"
          in
    List.iter (fun s -> ignore (stmt builder func_scope s); ()) fdecl.sbody in
